...
internal sealed class UtilityValidateAddressInfo : BaseEthereumOperationInfo
{
    ...
    public (string Code, string Message)? Validate() =>
        string.IsNullOrWhiteSpace(Address) ? ("VAL_MISSING_ADDRESS", "Config key 'address' is required for utility/validateAddress.") : null;
