...
{
    private async Task<NodeExecutionResult> _Ethereum_Utility_DecodeEventLog_Async(
        NodeExecutionContext nodeExecutionContext,
        CancellationToken cancellationToken = default)
    {
        //code-step: 1.1 - Validate settings exist and cast to UtilityDecodeEventLogInfo
        if (mySettings?.ActiveInfo is not UtilityDecodeEventLogInfo info)
        ...
        try
        {
            //code-step: 1.3 - Call Ethereum utility service to decode the event log (load-bearing for SC06/TRIG03 decoded output)
            var r = await _utilityService.DecodeEventLogAsync(info.EventAbi!, info.Topics!, info.Data!, cancellationToken);
            ...
            //code-step: 1.5 - Extract decoded event record from result
            ...
            outputData["operation"] = "decodeEventLog";

            //code-step: 1.7 - Convert decoded event record to standard items array
        ...
        catch (Exception ex)
        {
            ...
            return resultManager.SetResultAsError(ExecutionConstants.OutputPorts.Error, $"utility/decodeEventLog failed: {ex.Message}", this);
        }
    }
}
