...
{
    private async Task<NodeExecutionResult> _Ethereum_Utility_ChainId_Async(
        NodeExecutionContext nodeExecutionContext,
        CancellationToken cancellationToken = default)
    {
        //code-step: 1.1 - Validate settings exist and cast to UtilityChainIdInfo
        if (mySettings?.ActiveInfo is not UtilityChainIdInfo info)
        ...
        try
        {
            //code-step: 1.3 - Call Ethereum utility service to fetch chain ID
            var r = await _utilityService.GetChainIdAsync(info.Network, cancellationToken);
            ...
            //code-step: 1.5 - Extract chain ID record from result
            var chainID = new Dictionary<string, object> { { "chainId", r.ChainID } };
            ...
            outputData["operation"] = "chainId";

            //code-step: 1.7 - Convert chain ID record to standard items array
            outputData.TryGetValue(ExecutionConstants.OutputFieldNameConstants.CONST_items, out var existingItemsValue);
            outputData[ExecutionConstants.OutputFieldNameConstants.CONST_items] = ApplyOutputItemsMerge(existingItemsValue, WrapJsonIntoItems(chainID, nodeExecutionContext));

            //code-step: 1.8 - Write output (handles TargetDataPath writes + items downstream)
            return await WriteOutputData(ExecutionConstants.OutputPorts.Success, outputData, chainID, nodeExecutionContext, cancellationToken);
        }
        catch (OperationCanceledException) { throw; }
        catch (Exception ex)
        {
            //code-step: 1.9 - Catch exceptions and return error with context
            return resultManager.SetResultAsError(ExecutionConstants.OutputPorts.Error, $"utility/chainId failed on {info.Network}: {ex.Message}", this);
        }
    }
}
