...
{
    private async Task<NodeExecutionResult> _Ethereum_Utility_ContractAddress_Async(
        NodeExecutionContext nodeExecutionContext,
        CancellationToken cancellationToken = default)
    {
        //code-step: 1.1 - Validate settings exist and cast to UtilityContractAddressInfo
        if (mySettings?.ActiveInfo is not UtilityContractAddressInfo info)
        ...
        try
        {
            //code-step: 1.3 - Call Ethereum utility service to predict the CREATE/CREATE2 contract address
            var r = await _utilityService.GetContractAddressAsync(info.From!, info.Nonce, info.BytecodeHash, info.Salt, cancellationToken);
            ...
            //code-step: 1.5 - Extract predicted address record from result
            ...
            outputData["operation"] = "contractAddress";

            //code-step: 1.7 - Convert predicted address record to standard items array
            outputData.TryGetValue(ExecutionConstants.OutputFieldNameConstants.CONST_items, out var existingItemsValue);
            outputData[ExecutionConstants.OutputFieldNameConstants.CONST_items] = ApplyOutputItemsMerge(existingItemsValue, WrapJsonIntoItems(predicted, nodeExecutionContext));

            //code-step: 1.8 - Write output (handles TargetDataPath writes + items downstream)
            return await WriteOutputData(ExecutionConstants.OutputPorts.Success, outputData, predicted, nodeExecutionContext, cancellationToken);
        }
        catch (OperationCanceledException) { throw; }
        catch (Exception ex)
        {
            //code-step: 1.9 - Catch exceptions and return error with context
            return resultManager.SetResultAsError(ExecutionConstants.OutputPorts.Error, $"utility/contractAddress failed for {info.From}: {ex.Message}", this);
        }
    }
}
