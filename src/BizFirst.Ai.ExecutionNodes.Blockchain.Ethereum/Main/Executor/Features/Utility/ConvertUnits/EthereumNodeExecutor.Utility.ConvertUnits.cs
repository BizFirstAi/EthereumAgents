...
{
    private async Task<NodeExecutionResult> _Ethereum_Utility_ConvertUnits_Async(
        NodeExecutionContext nodeExecutionContext,
        CancellationToken cancellationToken = default)
    {
        //code-step: 1.1 - Validate settings exist and cast to UtilityConvertUnitsInfo
        if (mySettings?.ActiveInfo is not UtilityConvertUnitsInfo info)
        ...
        try
        {
            //code-step: 1.3 - Call Ethereum utility service to convert units
            var r = await _utilityService.ConvertUnitsAsync(info.Value!, info.FromUnit!, info.ToUnit!, cancellationToken);
            ...
            //code-step: 1.5 - Extract conversion record from result
            ...
            outputData["operation"] = "convertUnits";

            //code-step: 1.7 - Convert conversion record to standard items array
            outputData.TryGetValue(ExecutionConstants.OutputFieldNameConstants.CONST_items, out var existingItemsValue);
            outputData[ExecutionConstants.OutputFieldNameConstants.CONST_items] = ApplyOutputItemsMerge(existingItemsValue, WrapJsonIntoItems(conversion, nodeExecutionContext));

            //code-step: 1.8 - Write output (handles TargetDataPath writes + items downstream)
            return await WriteOutputData(ExecutionConstants.OutputPorts.Success, outputData, conversion, nodeExecutionContext, cancellationToken);
        }
        catch (OperationCanceledException) { throw; }
        catch (Exception ex)
        {
            //code-step: 1.9 - Catch exceptions and return error with context
            return resultManager.SetResultAsError(ExecutionConstants.OutputPorts.Error, $"utility/convertUnits failed for {info.Value}: {ex.Message}", this);
        }
    }
}
