...
{
    private async Task<NodeExecutionResult> _Ethereum_Utility_EncodeEventTopics_Async(
        NodeExecutionContext nodeExecutionContext,
        CancellationToken cancellationToken = default)
    {
        //code-step: 1.1 - Validate settings exist and cast to UtilityEncodeEventTopicsInfo
        if (mySettings?.ActiveInfo is not UtilityEncodeEventTopicsInfo info)
        ...
        try
        {
            //code-step: 1.3 - Call Ethereum utility service to encode event topics
            var r = await _utilityService.EncodeEventTopicsAsync(info.EventAbi!, info.Args, cancellationToken);
            ...
            //code-step: 1.5 - Extract encoded topics record from result
            var encoded = new Dictionary<string, object> { { "topics", r.Topics } };
            ...
            outputData["operation"] = "encodeEventTopics";

            //code-step: 1.7 - Convert encoded topics record to standard items array
        ...
        catch (Exception ex)
        {
            ...
            return resultManager.SetResultAsError(ExecutionConstants.OutputPorts.Error, $"utility/encodeEventTopics failed: {ex.Message}", this);
        }
    }
}
