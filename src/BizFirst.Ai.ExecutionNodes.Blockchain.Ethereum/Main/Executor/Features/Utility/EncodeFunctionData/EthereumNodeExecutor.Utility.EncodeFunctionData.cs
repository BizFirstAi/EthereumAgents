...
{
    private async Task<NodeExecutionResult> _Ethereum_Utility_EncodeFunctionData_Async(
        NodeExecutionContext nodeExecutionContext,
        CancellationToken cancellationToken = default)
    {
        //code-step: 1.1 - Validate settings exist and cast to UtilityEncodeFunctionDataInfo
        if (mySettings?.ActiveInfo is not UtilityEncodeFunctionDataInfo info)
        ...
        try
        {
            //code-step: 1.3 - Call Ethereum utility service to encode function calldata
            var r = await _utilityService.EncodeFunctionDataAsync(info.Abi!, info.FunctionName!, info.Args, cancellationToken);
            ...
            //code-step: 1.5 - Extract encoded data record from result
            var encoded = new Dictionary<string, object> { { "data", r.Data }, { "selector", r.Selector } };
            ...
            outputData["operation"] = "encodeFunctionData";

            //code-step: 1.7 - Convert encoded data record to standard items array
            outputData.TryGetValue(ExecutionConstants.OutputFieldNameConstants.CONST_items, out var existingItemsValue);
            outputData[ExecutionConstants.OutputFieldNameConstants.CONST_items] = ApplyOutputItemsMerge(existingItemsValue, WrapJsonIntoItems(encoded, nodeExecutionContext));

            //code-step: 1.8 - Write output (handles TargetDataPath writes + items downstream)
            return await WriteOutputData(ExecutionConstants.OutputPorts.Success, outputData, encoded, nodeExecutionContext, cancellationToken);
        }
        catch (OperationCanceledException) { throw; }
        catch (Exception ex)
        {
            //code-step: 1.9 - Catch exceptions and return error with context
            return resultManager.SetResultAsError(ExecutionConstants.OutputPorts.Error, $"utility/encodeFunctionData failed for {info.FunctionName}: {ex.Message}", this);
        }
    }
}
