...
{
    private async Task<NodeExecutionResult> _Ethereum_Nft_Approve_Async(
        NodeExecutionContext nodeExecutionContext,
        CancellationToken cancellationToken = default)
    {
        //code-step: 1.1 - Validate settings exist and cast to NftApproveInfo
        if (mySettings?.ActiveInfo is not NftApproveInfo info)
        ...
        try
        {
            //code-step: 1.3 - Resolve signing credential and call Ethereum nft service to approve a spender for the token
            ...
            var r = await _nftService.ApproveAsync(info.Network, privateKey!, info.ContractAddress!, info.Spender!, tokenID, cancellationToken);
            ...
            var approve = new Dictionary<string, object> { { "txHash", r.TxHash } };
            ...
            outputData[ExecutionConstants.OutputFieldNameConstants.CONST_items] = ApplyOutputItemsMerge(existingItemsValue, WrapJsonIntoItems(approve, nodeExecutionContext));

            //code-step: 1.8 - Write output (handles TargetDataPath writes + items downstream)
            return await WriteOutputData(ExecutionConstants.OutputPorts.Success, outputData, approve, nodeExecutionContext, cancellationToken);
        }
        catch (OperationCanceledException) { throw; }
        catch (Exception ex)
        {
            //code-step: 1.9 - Catch exceptions and return error with context
            return resultManager.SetResultAsError(ExecutionConstants.OutputPorts.Error, $"nft/approve failed for {info.ContractAddress} tokenID {info.TokenID}: {ex.Message}", this);
        }
    }
}
