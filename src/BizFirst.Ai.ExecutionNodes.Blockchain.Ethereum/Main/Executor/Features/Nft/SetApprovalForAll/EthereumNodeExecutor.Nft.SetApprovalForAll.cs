...
{
    private async Task<NodeExecutionResult> _Ethereum_Nft_SetApprovalForAll_Async(
        NodeExecutionContext nodeExecutionContext,
        CancellationToken cancellationToken = default)
    {
        //code-step: 1.1 - Validate settings exist and cast to NftSetApprovalForAllInfo
        if (mySettings?.ActiveInfo is not NftSetApprovalForAllInfo info)
        ...
        try
        {
            //code-step: 1.3 - Resolve signing credential and call Ethereum nft service to set operator-for-all approval
            var (privateKey, credError) = await _ResolveSigningKeyAsync(cancellationToken);
            if (credError is not null)
                return credError;

            var r = await _nftService.SetApprovalForAllAsync(info.Network, privateKey!, info.ContractAddress!, info.Operator!, info.Approved, cancellationToken);
            ...
            var setApproval = new Dictionary<string, object> { { "txHash", r.TxHash } };
            ...
            outputData["operation"] = "setApprovalForAll";
            ...
            outputData[ExecutionConstants.OutputFieldNameConstants.CONST_items] = ApplyOutputItemsMerge(existingItemsValue, WrapJsonIntoItems(setApproval, nodeExecutionContext));

            //code-step: 1.8 - Write output (handles TargetDataPath writes + items downstream)
            return await WriteOutputData(ExecutionConstants.OutputPorts.Success, outputData, setApproval, nodeExecutionContext, cancellationToken);
        }
        catch (OperationCanceledException) { throw; }
        catch (Exception ex)
        {
            //code-step: 1.9 - Catch exceptions and return error with context
            return resultManager.SetResultAsError(ExecutionConstants.OutputPorts.Error, $"nft/setApprovalForAll failed for {info.ContractAddress}: {ex.Message}", this);
        }
    }
}
