...
{
    private async Task<NodeExecutionResult> _Ethereum_Nft_Balance_Async(
        NodeExecutionContext nodeExecutionContext,
        CancellationToken cancellationToken = default)
    {
        //code-step: 1.1 - Validate settings exist and cast to NftBalanceInfo
        if (mySettings?.ActiveInfo is not NftBalanceInfo info)
        ...
        try
        {
            //code-step: 1.3 - Call Ethereum nft service to fetch balance
            var r = await _nftService.GetBalanceAsync(info.Network, info.ContractAddress!, info.OwnerAddress!, cancellationToken);
            ...
            var balance = new Dictionary<string, object> { { "balance", r.Balance } };
        ...
        catch (Exception ex)
        {
            ...
            return resultManager.SetResultAsError(ExecutionConstants.OutputPorts.Error, $"nft/balance failed for {info.ContractAddress}: {ex.Message}", this);
        }
    }
}
