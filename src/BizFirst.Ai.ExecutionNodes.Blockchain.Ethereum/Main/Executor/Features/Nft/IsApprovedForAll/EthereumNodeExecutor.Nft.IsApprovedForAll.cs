...
{
    private async Task<NodeExecutionResult> _Ethereum_Nft_IsApprovedForAll_Async(
        NodeExecutionContext nodeExecutionContext,
        CancellationToken cancellationToken = default)
    {
        //code-step: 1.1 - Validate settings exist and cast to NftIsApprovedForAllInfo
        if (mySettings?.ActiveInfo is not NftIsApprovedForAllInfo info)
        ...
        try
        {
            //code-step: 1.3 - Call Ethereum nft service to check operator-for-all approval
            var r = await _nftService.IsApprovedForAllAsync(info.Network, info.ContractAddress!, info.Owner!, info.Operator!, cancellationToken);
            ...
            //code-step: 1.5 - Extract approval record from result
            var isApproved = new Dictionary<string, object> { { "isApproved", r.IsApproved } };
            ...
            outputData["operation"] = "isApprovedForAll";

            //code-step: 1.7 - Convert approval record to standard items array
            outputData.TryGetValue(ExecutionConstants.OutputFieldNameConstants.CONST_items, out var existingItemsValue);
            outputData[ExecutionConstants.OutputFieldNameConstants.CONST_items] = ApplyOutputItemsMerge(existingItemsValue, WrapJsonIntoItems(isApproved, nodeExecutionContext));

            //code-step: 1.8 - Write output (handles TargetDataPath writes + items downstream)
            return await WriteOutputData(ExecutionConstants.OutputPorts.Success, outputData, isApproved, nodeExecutionContext, cancellationToken);
        }
        catch (OperationCanceledException) { throw; }
        catch (Exception ex)
        {
            //code-step: 1.9 - Catch exceptions and return error with context
            return resultManager.SetResultAsError(ExecutionConstants.OutputPorts.Error, $"nft/isApprovedForAll failed for {info.ContractAddress}: {ex.Message}", this);
        }
    }
}
