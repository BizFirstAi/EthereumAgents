...
{
    private async Task<NodeExecutionResult> _Ethereum_Nft_OwnerOf_Async(
        NodeExecutionContext nodeExecutionContext,
        CancellationToken cancellationToken = default)
    {
        //code-step: 1.1 - Validate settings exist and cast to NftOwnerOfInfo
        if (mySettings?.ActiveInfo is not NftOwnerOfInfo info)
        ...
        try
        {
            //code-step: 1.3 - Call Ethereum nft service to fetch the owner of a token
            var tokenID = BigInteger.Parse(info.TokenID!);
            var r = await _nftService.GetOwnerOfAsync(info.Network, info.ContractAddress!, tokenID, cancellationToken);
            ...
            //code-step: 1.5 - Extract owner record from result
            var owner = new Dictionary<string, object> { { "owner", r.Owner } };
            ...
            outputData["operation"] = "ownerOf";

            //code-step: 1.7 - Convert owner record to standard items array
            outputData.TryGetValue(ExecutionConstants.OutputFieldNameConstants.CONST_items, out var existingItemsValue);
            outputData[ExecutionConstants.OutputFieldNameConstants.CONST_items] = ApplyOutputItemsMerge(existingItemsValue, WrapJsonIntoItems(owner, nodeExecutionContext));

            //code-step: 1.8 - Write output (handles TargetDataPath writes + items downstream)
            return await WriteOutputData(ExecutionConstants.OutputPorts.Success, outputData, owner, nodeExecutionContext, cancellationToken);
        }
        catch (OperationCanceledException) { throw; }
        catch (Exception ex)
        {
            //code-step: 1.9 - Catch exceptions and return error with context
            return resultManager.SetResultAsError(ExecutionConstants.OutputPorts.Error, $"nft/ownerOf failed for {info.ContractAddress} tokenID {info.TokenID}: {ex.Message}", this);
        }
    }
}
