...
{
    private async Task<NodeExecutionResult> _Ethereum_Nft_TransferFrom_Async(
        NodeExecutionContext nodeExecutionContext,
        CancellationToken cancellationToken = default)
    {
        //code-step: 1.1 - Validate settings exist and cast to NftTransferFromInfo
        if (mySettings?.ActiveInfo is not NftTransferFromInfo info)
        ...
        try
        {
            //code-step: 1.3 - Resolve signing credential and call Ethereum nft service to transfer the token
            ...
            var r = await _nftService.TransferFromAsync(info.Network, privateKey!, info.ContractAddress!, info.From!, info.To!, tokenID, cancellationToken);
            ...
            outputData["operation"] = "transferFrom";
        ...
        catch (Exception ex)
        {
            ...
            return resultManager.SetResultAsError(ExecutionConstants.OutputPorts.Error, $"nft/transferFrom failed for {info.ContractAddress} tokenID {info.TokenID}: {ex.Message}", this);
        }
    }
}
