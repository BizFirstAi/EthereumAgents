...
{
    private async Task<NodeExecutionResult> _Ethereum_Nft_SafeTransferFrom_Async(
        NodeExecutionContext nodeExecutionContext,
        CancellationToken cancellationToken = default)
    {
        //code-step: 1.1 - Validate settings exist and cast to NftSafeTransferFromInfo
        if (mySettings?.ActiveInfo is not NftSafeTransferFromInfo info)
        ...
        try
        {
            //code-step: 1.3 - Resolve signing credential and call Ethereum nft service to safely transfer the token
            ...
            var r = await _nftService.SafeTransferFromAsync(info.Network, privateKey!, info.ContractAddress!, info.From!, info.To!, tokenID, cancellationToken);
            ...
            outputData["operation"] = "safeTransferFrom";
        ...
        catch (Exception ex)
        {
            ...
            return resultManager.SetResultAsError(ExecutionConstants.OutputPorts.Error, $"nft/safeTransferFrom failed for {info.ContractAddress} tokenID {info.TokenID}: {ex.Message}", this);
        }
    }
}
