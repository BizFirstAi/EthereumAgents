...
{
    private async Task<NodeExecutionResult> _Ethereum_Nft_GetApproved_Async(
        NodeExecutionContext nodeExecutionContext,
        CancellationToken cancellationToken = default)
    {
        //code-step: 1.1 - Validate settings exist and cast to NftGetApprovedInfo
        if (mySettings?.ActiveInfo is not NftGetApprovedInfo info)
        ...
        try
        {
            //code-step: 1.3 - Call Ethereum nft service to fetch the approved address for a token
            var tokenID = BigInteger.Parse(info.TokenID!);
            var r = await _nftService.GetApprovedAsync(info.Network, info.ContractAddress!, tokenID, cancellationToken);
            ...
            //code-step: 1.5 - Extract approved record from result
            var approved = new Dictionary<string, object> { { "approved", r.Approved } };
            ...
            outputData["operation"] = "getApproved";

            //code-step: 1.7 - Convert approved record to standard items array
            outputData.TryGetValue(ExecutionConstants.OutputFieldNameConstants.CONST_items, out var existingItemsValue);
            outputData[ExecutionConstants.OutputFieldNameConstants.CONST_items] = ApplyOutputItemsMerge(existingItemsValue, WrapJsonIntoItems(approved, nodeExecutionContext));

            //code-step: 1.8 - Write output (handles TargetDataPath writes + items downstream)
            return await WriteOutputData(ExecutionConstants.OutputPorts.Success, outputData, approved, nodeExecutionContext, cancellationToken);
        }
        catch (OperationCanceledException) { throw; }
        catch (Exception ex)
        {
            //code-step: 1.9 - Catch exceptions and return error with context
            return resultManager.SetResultAsError(ExecutionConstants.OutputPorts.Error, $"nft/getApproved failed for {info.ContractAddress} tokenID {info.TokenID}: {ex.Message}", this);
        }
    }
}
