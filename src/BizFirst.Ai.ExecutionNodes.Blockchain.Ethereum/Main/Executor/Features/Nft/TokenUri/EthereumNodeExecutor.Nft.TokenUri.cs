...
{
    private async Task<NodeExecutionResult> _Ethereum_Nft_TokenUri_Async(
        NodeExecutionContext nodeExecutionContext,
        CancellationToken cancellationToken = default)
    {
        //code-step: 1.1 - Validate settings exist and cast to NftTokenUriInfo
        if (mySettings?.ActiveInfo is not NftTokenUriInfo info)
        ...
        try
        {
            //code-step: 1.3 - Call Ethereum nft service to fetch the token URI (raw, unresolved)
            var tokenID = BigInteger.Parse(info.TokenID!);
            var r = await _nftService.GetTokenUriAsync(info.Network, info.ContractAddress!, tokenID, cancellationToken);
            ...
            //code-step: 1.5 - Extract tokenUri record from result
            var tokenUri = new Dictionary<string, object> { { "tokenUri", r.TokenUri } };
            ...
            outputData["operation"] = "tokenUri";

            //code-step: 1.7 - Convert tokenUri record to standard items array
            outputData.TryGetValue(ExecutionConstants.OutputFieldNameConstants.CONST_items, out var existingItemsValue);
            outputData[ExecutionConstants.OutputFieldNameConstants.CONST_items] = ApplyOutputItemsMerge(existingItemsValue, WrapJsonIntoItems(tokenUri, nodeExecutionContext));

            //code-step: 1.8 - Write output (handles TargetDataPath writes + items downstream)
            return await WriteOutputData(ExecutionConstants.OutputPorts.Success, outputData, tokenUri, nodeExecutionContext, cancellationToken);
        }
        catch (OperationCanceledException) { throw; }
        catch (Exception ex)
        {
            //code-step: 1.9 - Catch exceptions and return error with context
            return resultManager.SetResultAsError(ExecutionConstants.OutputPorts.Error, $"nft/tokenUri failed for {info.ContractAddress} tokenID {info.TokenID}: {ex.Message}", this);
        }
    }
}
