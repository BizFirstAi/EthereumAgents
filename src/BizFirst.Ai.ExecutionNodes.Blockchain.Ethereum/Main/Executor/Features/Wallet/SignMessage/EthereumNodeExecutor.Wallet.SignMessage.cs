...
{
    private async Task<NodeExecutionResult> _Ethereum_Wallet_SignMessage_Async(
        NodeExecutionContext nodeExecutionContext,
        CancellationToken cancellationToken = default)
    {
        //code-step: 1.1 - Validate settings exist and cast to WalletSignMessageInfo
        if (mySettings?.ActiveInfo is not WalletSignMessageInfo info)
        ...
        try
        {
            //code-step: 1.3 - Resolve signing credential and call Ethereum wallet service to sign the message (EIP-191)
            var (privateKey, credError) = await _ResolveSigningKeyAsync(cancellationToken);
            if (credError is not null)
                return credError;

            var r = await _walletService.SignMessageAsync(privateKey!, info.Message!, cancellationToken);
            ...
            outputData["operation"] = "signMessage";
        ...
        catch (Exception ex)
        {
            ...
            return resultManager.SetResultAsError(ExecutionConstants.OutputPorts.Error, $"wallet/signMessage failed: {ex.Message}", this);
        }
    }
}
