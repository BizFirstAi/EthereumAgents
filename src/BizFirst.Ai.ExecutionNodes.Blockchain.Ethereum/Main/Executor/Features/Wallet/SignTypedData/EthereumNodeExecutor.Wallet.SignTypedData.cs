...
{
    private async Task<NodeExecutionResult> _Ethereum_Wallet_SignTypedData_Async(
        NodeExecutionContext nodeExecutionContext,
        CancellationToken cancellationToken = default)
    {
        //code-step: 1.1 - Validate settings exist and cast to WalletSignTypedDataInfo
        if (mySettings?.ActiveInfo is not WalletSignTypedDataInfo info)
        ...
        try
        {
            //code-step: 1.3 - Resolve signing credential and call Ethereum wallet service to sign the typed data (EIP-712)
            var (privateKey, credError) = await _ResolveSigningKeyAsync(cancellationToken);
            if (credError is not null)
                return credError;

            var r = await _walletService.SignTypedDataAsync(privateKey!, info.TypedDataJson!, cancellationToken);
            ...
            //code-step: 1.5 - Extract signature record from result
            var signed = new Dictionary<string, object> { { "address", r.Address }, { "signature", r.Signature } };
            ...
            outputData["operation"] = "signTypedData";

            //code-step: 1.7 - Convert signature record to standard items array
            outputData.TryGetValue(ExecutionConstants.OutputFieldNameConstants.CONST_items, out var existingItemsValue);
            outputData[ExecutionConstants.OutputFieldNameConstants.CONST_items] = ApplyOutputItemsMerge(existingItemsValue, WrapJsonIntoItems(signed, nodeExecutionContext));

            //code-step: 1.8 - Write output (handles TargetDataPath writes + items downstream)
            return await WriteOutputData(ExecutionConstants.OutputPorts.Success, outputData, signed, nodeExecutionContext, cancellationToken);
        }
        catch (OperationCanceledException) { throw; }
        catch (Exception ex)
        {
            //code-step: 1.9 - Catch exceptions and return error with context
            return resultManager.SetResultAsError(ExecutionConstants.OutputPorts.Error, $"wallet/signTypedData failed: {ex.Message}", this);
        }
    }
}
