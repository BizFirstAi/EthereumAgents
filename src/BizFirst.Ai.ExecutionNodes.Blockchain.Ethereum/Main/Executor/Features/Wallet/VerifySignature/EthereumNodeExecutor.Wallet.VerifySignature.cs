...
{
    private async Task<NodeExecutionResult> _Ethereum_Wallet_VerifySignature_Async(
        NodeExecutionContext nodeExecutionContext,
        CancellationToken cancellationToken = default)
    {
        //code-step: 1.1 - Validate settings exist and cast to WalletVerifySignatureInfo
        if (mySettings?.ActiveInfo is not WalletVerifySignatureInfo info)
        ...
        try
        {
            //code-step: 1.3 - Call Ethereum wallet service to recover the signer address (no credential required)
            var r = await _walletService.VerifySignatureAsync(info.Message!, info.Signature!, cancellationToken);
            ...
            //code-step: 1.5 - Extract recovered address record from result
            var recovered = new Dictionary<string, object> { { "address", r.Address } };

            //code-step: 1.6 - Build output metadata dictionary
            var outputData = resultManager.GetOrCreateOutputData();
            outputData["status"] = "success";
            outputData["resource"] = "wallet";
            outputData["operation"] = "verifySignature";

            //code-step: 1.7 - Convert recovered address record to standard items array
            outputData.TryGetValue(ExecutionConstants.OutputFieldNameConstants.CONST_items, out var existingItemsValue);
            outputData[ExecutionConstants.OutputFieldNameConstants.CONST_items] = ApplyOutputItemsMerge(existingItemsValue, WrapJsonIntoItems(recovered, nodeExecutionContext));

            //code-step: 1.8 - Write output (handles TargetDataPath writes + items downstream)
            return await WriteOutputData(ExecutionConstants.OutputPorts.Success, outputData, recovered, nodeExecutionContext, cancellationToken);
        }
        catch (OperationCanceledException) { throw; }
        catch (Exception ex)
        {
            //code-step: 1.9 - Catch exceptions and return error with context
            return resultManager.SetResultAsError(ExecutionConstants.OutputPorts.Error, $"wallet/verifySignature failed: {ex.Message}", this);
        }
    }
}
