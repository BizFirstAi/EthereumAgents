...
{
    private async Task<NodeExecutionResult> _Ethereum_Gas_Estimate_Async(
        NodeExecutionContext nodeExecutionContext,
        CancellationToken cancellationToken = default)
    {
        //code-step: 1.1 - Validate settings exist and cast to GasEstimateInfo
        if (mySettings?.ActiveInfo is not GasEstimateInfo info)
        ...
        try
        {
            //code-step: 1.3 - Call Ethereum gas service to estimate gas usage
            var r = await _gasService.EstimateGasAsync(info.Network, info.To!, info.Value, info.Data, info.From, cancellationToken);
            ...
            //code-step: 1.5 - Extract gas estimate record from result
            var estimate = new Dictionary<string, object> { { "gasUnits", r.GasUnits } };
            ...
            outputData["operation"] = "estimate";

            //code-step: 1.7 - Convert gas estimate record to standard items array
            outputData.TryGetValue(ExecutionConstants.OutputFieldNameConstants.CONST_items, out var existingItemsValue);
            outputData[ExecutionConstants.OutputFieldNameConstants.CONST_items] = ApplyOutputItemsMerge(existingItemsValue, WrapJsonIntoItems(estimate, nodeExecutionContext));

            //code-step: 1.8 - Write output (handles TargetDataPath writes + items downstream)
            return await WriteOutputData(ExecutionConstants.OutputPorts.Success, outputData, estimate, nodeExecutionContext, cancellationToken);
        }
        catch (OperationCanceledException) { throw; }
        catch (Exception ex)
        {
            //code-step: 1.9 - Catch exceptions and return error with context
            return resultManager.SetResultAsError(ExecutionConstants.OutputPorts.Error, $"gas/estimate failed for {info.To}: {ex.Message}", this);
        }
    }
}
