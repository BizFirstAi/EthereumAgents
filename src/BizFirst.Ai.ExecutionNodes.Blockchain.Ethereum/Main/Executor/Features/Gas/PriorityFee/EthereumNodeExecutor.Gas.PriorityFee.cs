...
{
    private async Task<NodeExecutionResult> _Ethereum_Gas_PriorityFee_Async(
        NodeExecutionContext nodeExecutionContext,
        CancellationToken cancellationToken = default)
    {
        //code-step: 1.1 - Validate settings exist and cast to GasPriorityFeeInfo
        if (mySettings?.ActiveInfo is not GasPriorityFeeInfo info)
        ...
        try
        {
            //code-step: 1.3 - Call Ethereum gas service to fetch the suggested max priority fee
            var r = await _gasService.GetMaxPriorityFeeAsync(info.Network, cancellationToken);
            ...
            //code-step: 1.5 - Extract priority fee record from result
            var priorityFee = new Dictionary<string, object>
            {
                { "priorityFeeWei", r.PriorityFeeWei },
                { "priorityFeeGwei", r.PriorityFeeGwei },
            };
            ...
            outputData["operation"] = "priorityFee";

            //code-step: 1.7 - Convert priority fee record to standard items array
            outputData.TryGetValue(ExecutionConstants.OutputFieldNameConstants.CONST_items, out var existingItemsValue);
            outputData[ExecutionConstants.OutputFieldNameConstants.CONST_items] = ApplyOutputItemsMerge(existingItemsValue, WrapJsonIntoItems(priorityFee, nodeExecutionContext));

            //code-step: 1.8 - Write output (handles TargetDataPath writes + items downstream)
            return await WriteOutputData(ExecutionConstants.OutputPorts.Success, outputData, priorityFee, nodeExecutionContext, cancellationToken);
        }
        catch (OperationCanceledException) { throw; }
        catch (Exception ex)
        {
            //code-step: 1.9 - Catch exceptions and return error with context
            return resultManager.SetResultAsError(ExecutionConstants.OutputPorts.Error, $"gas/priorityFee failed on {info.Network}: {ex.Message}", this);
        }
    }
}
