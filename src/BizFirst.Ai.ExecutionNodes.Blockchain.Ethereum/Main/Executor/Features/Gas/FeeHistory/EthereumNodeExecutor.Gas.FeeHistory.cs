...
{
    private async Task<NodeExecutionResult> _Ethereum_Gas_FeeHistory_Async(
        NodeExecutionContext nodeExecutionContext,
        CancellationToken cancellationToken = default)
    {
        //code-step: 1.1 - Validate settings exist and cast to GasFeeHistoryInfo
        if (mySettings?.ActiveInfo is not GasFeeHistoryInfo info)
        ...
        try
        {
            //code-step: 1.3 - Call Ethereum gas service to fetch fee history
            var r = await _gasService.GetFeeHistoryAsync(info.Network, info.BlockCount!.Value, info.NewestBlock, info.RewardPercentiles, cancellationToken);
            ...
            //code-step: 1.5 - Extract per-block fee history list from result
            var blocks = r.Blocks;
            ...
            outputData["operation"] = "feeHistory";
            outputData["oldestBlock"] = r.OldestBlock;
            outputData["count"] = blocks.Count;

            //code-step: 1.7 - Convert fee history list to standard items array
        ...
        catch (Exception ex)
        {
            ...
            return resultManager.SetResultAsError(ExecutionConstants.OutputPorts.Error, $"gas/feeHistory failed on {info.Network}: {ex.Message}", this);
        }
    }
}
