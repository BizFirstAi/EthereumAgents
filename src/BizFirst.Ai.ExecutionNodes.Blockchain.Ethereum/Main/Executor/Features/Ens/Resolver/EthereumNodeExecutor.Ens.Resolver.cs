...
{
    private async Task<NodeExecutionResult> _Ethereum_Ens_Resolver_Async(
        NodeExecutionContext nodeExecutionContext,
        CancellationToken cancellationToken = default)
    {
        //code-step: 1.1 - Validate settings exist and cast to EnsResolverInfo
        if (mySettings?.ActiveInfo is not EnsResolverInfo info)
        ...
        try
        {
            //code-step: 1.3 - Call Ethereum ENS service to look up the resolver contract address
            var r = await _ensService.GetResolverAsync(info.Network, info.Name!, cancellationToken);
            ...
            //code-step: 1.5 - Extract resolver record from result
            var resolver = new Dictionary<string, object>
            {
                { "name", info.Name! },
            ...
            outputData["operation"] = "resolver";

            //code-step: 1.7 - Convert resolver record to standard items array
            outputData.TryGetValue(ExecutionConstants.OutputFieldNameConstants.CONST_items, out var existingItemsValue);
            outputData[ExecutionConstants.OutputFieldNameConstants.CONST_items] = ApplyOutputItemsMerge(existingItemsValue, WrapJsonIntoItems(resolver, nodeExecutionContext));

            //code-step: 1.8 - Write output (handles TargetDataPath writes + items downstream)
            return await WriteOutputData(ExecutionConstants.OutputPorts.Success, outputData, resolver, nodeExecutionContext, cancellationToken);
        }
        catch (OperationCanceledException) { throw; }
        catch (Exception ex)
        {
            //code-step: 1.9 - Catch exceptions and return error with context
            return resultManager.SetResultAsError(ExecutionConstants.OutputPorts.Error, $"ens/resolver failed for {info.Name}: {ex.Message}", this);
        }
    }
}
