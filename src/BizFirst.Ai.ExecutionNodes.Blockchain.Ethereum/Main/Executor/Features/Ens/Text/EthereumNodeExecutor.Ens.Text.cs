...
{
    private async Task<NodeExecutionResult> _Ethereum_Ens_Text_Async(
        NodeExecutionContext nodeExecutionContext,
        CancellationToken cancellationToken = default)
    {
        //code-step: 1.1 - Validate settings exist and cast to EnsTextInfo
        if (mySettings?.ActiveInfo is not EnsTextInfo info)
        ...
        try
        {
            //code-step: 1.3 - Call Ethereum ENS service to fetch the requested text record
            var r = await _ensService.GetTextAsync(info.Network, info.Name!, info.Key!, cancellationToken);
            ...
            //code-step: 1.5 - Extract text record from result
            ...
            outputData["operation"] = "text";

            //code-step: 1.7 - Convert text record to standard items array
            outputData.TryGetValue(ExecutionConstants.OutputFieldNameConstants.CONST_items, out var existingItemsValue);
            outputData[ExecutionConstants.OutputFieldNameConstants.CONST_items] = ApplyOutputItemsMerge(existingItemsValue, WrapJsonIntoItems(text, nodeExecutionContext));

            //code-step: 1.8 - Write output (handles TargetDataPath writes + items downstream)
            return await WriteOutputData(ExecutionConstants.OutputPorts.Success, outputData, text, nodeExecutionContext, cancellationToken);
        }
        catch (OperationCanceledException) { throw; }
        catch (Exception ex)
        {
            //code-step: 1.9 - Catch exceptions and return error with context
            return resultManager.SetResultAsError(ExecutionConstants.OutputPorts.Error, $"ens/text failed for {info.Name}/{info.Key}: {ex.Message}", this);
        }
    }
}
