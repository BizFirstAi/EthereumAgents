...
{
    private async Task<NodeExecutionResult> _Ethereum_Block_List_Async(
        NodeExecutionContext nodeExecutionContext,
        CancellationToken cancellationToken = default)
    {
        //code-step: 1.1 - Validate settings exist and cast to BlockListInfo
        if (mySettings?.ActiveInfo is not BlockListInfo info)
        ...
        try
        {
            //code-step: 1.3 - Call Ethereum block service to list recent blocks
            var r = await _blockService.ListRecentBlocksAsync(info.Network, info.Count, info.FromBlock, cancellationToken);
            ...
            //code-step: 1.5 - Extract block list from result
            var blocks = r.Blocks;

            //code-step: 1.6 - Build output metadata dictionary
            var outputData = resultManager.GetOrCreateOutputData();
            outputData["status"] = "success";
            outputData["resource"] = "block";
            outputData["operation"] = "list";
            outputData["count"] = blocks.Count;

            //code-step: 1.7 - Convert block list to standard items array
            outputData.TryGetValue(ExecutionConstants.OutputFieldNameConstants.CONST_items, out var existingItemsValue);
            outputData[ExecutionConstants.OutputFieldNameConstants.CONST_items] = ApplyOutputItemsMerge(existingItemsValue, WrapJsonIntoItems(blocks, nodeExecutionContext));

            //code-step: 1.8 - Write output (handles TargetDataPath writes + items downstream)
            return await WriteOutputData(ExecutionConstants.OutputPorts.Success, outputData, blocks, nodeExecutionContext, cancellationToken);
        }
        catch (OperationCanceledException) { throw; }
        catch (Exception ex)
        {
            //code-step: 1.9 - Catch exceptions and return error with context
            return resultManager.SetResultAsError(ExecutionConstants.OutputPorts.Error, $"block/list failed on {info.Network ?? "default"}: {ex.Message}", this);
        }
    }
}
