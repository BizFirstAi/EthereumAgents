...
{
    private async Task<NodeExecutionResult> _Ethereum_Block_Get_Async(
        NodeExecutionContext nodeExecutionContext,
        CancellationToken cancellationToken = default)
    {
        //code-step: 1.1 - Validate settings exist and cast to BlockGetInfo
        if (mySettings?.ActiveInfo is not BlockGetInfo info)
        ...
        try
        {
            //code-step: 1.3 - Call Ethereum block service to fetch block
            var r = await _blockService.GetBlockAsync(info.Network, info.BlockNumberOrHash, info.IncludeTransactions, cancellationToken);
            ...
            //code-step: 1.5 - Extract block record from result
            var block = r.Block!;
            ...
            //code-step: 1.7 - Convert block record to standard items array
            outputData.TryGetValue(ExecutionConstants.OutputFieldNameConstants.CONST_items, out var existingItemsValue);
            outputData[ExecutionConstants.OutputFieldNameConstants.CONST_items] = ApplyOutputItemsMerge(existingItemsValue, WrapJsonIntoItems(block, nodeExecutionContext));

            //code-step: 1.8 - Write output (handles TargetDataPath writes + items downstream)
            return await WriteOutputData(ExecutionConstants.OutputPorts.Success, outputData, block, nodeExecutionContext, cancellationToken);
        }
        catch (OperationCanceledException) { throw; }
        catch (Exception ex)
        {
            //code-step: 1.9 - Catch exceptions and return error with context
            return resultManager.SetResultAsError(ExecutionConstants.OutputPorts.Error, $"block/get failed for {info.BlockNumberOrHash ?? "latest"}: {ex.Message}", this);
        }
    }
}
