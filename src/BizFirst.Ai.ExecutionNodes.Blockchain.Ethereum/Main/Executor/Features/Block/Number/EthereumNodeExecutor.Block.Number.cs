...
{
    private async Task<NodeExecutionResult> _Ethereum_Block_Number_Async(
        NodeExecutionContext nodeExecutionContext,
        CancellationToken cancellationToken = default)
    {
        //code-step: 1.1 - Validate settings exist and cast to BlockNumberInfo
        if (mySettings?.ActiveInfo is not BlockNumberInfo info)
        ...
        try
        {
            //code-step: 1.3 - Call Ethereum block service to fetch current block number
            var r = await _blockService.GetBlockNumberAsync(info.Network, cancellationToken);
            ...
            //code-step: 1.5 - Extract block number record from result
            var blockNumber = new Dictionary<string, object> { { "blockNumber", r.BlockNumber } };
            ...
            outputData["operation"] = "number";

            //code-step: 1.7 - Convert block number record to standard items array
            outputData.TryGetValue(ExecutionConstants.OutputFieldNameConstants.CONST_items, out var existingItemsValue);
            outputData[ExecutionConstants.OutputFieldNameConstants.CONST_items] = ApplyOutputItemsMerge(existingItemsValue, WrapJsonIntoItems(blockNumber, nodeExecutionContext));

            //code-step: 1.8 - Write output (handles TargetDataPath writes + items downstream)
            return await WriteOutputData(ExecutionConstants.OutputPorts.Success, outputData, blockNumber, nodeExecutionContext, cancellationToken);
        }
        catch (OperationCanceledException) { throw; }
        catch (Exception ex)
        {
            //code-step: 1.9 - Catch exceptions and return error with context
            return resultManager.SetResultAsError(ExecutionConstants.OutputPorts.Error, $"block/number failed on {info.Network ?? "default"}: {ex.Message}", this);
        }
    }
}
