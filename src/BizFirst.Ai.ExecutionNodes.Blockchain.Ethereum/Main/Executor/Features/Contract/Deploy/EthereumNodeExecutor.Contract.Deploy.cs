...
{
    private async Task<NodeExecutionResult> _Ethereum_Contract_Deploy_Async(
        NodeExecutionContext nodeExecutionContext,
        CancellationToken cancellationToken = default)
    {
        //code-step: 1.1 - Validate settings exist and cast to ContractDeployInfo
        if (mySettings?.ActiveInfo is not ContractDeployInfo info)
        ...
        try
        {
            //code-step: 1.3 - Resolve signing key and call Ethereum contract service to deploy the contract
            var (privateKey, credError) = await _ResolveSigningKeyAsync(cancellationToken);
            if (credError is not null)
                return credError;

            var r = await _contractService.DeployAsync(info.Network, privateKey!, info.Bytecode!, info.Abi!, info.ConstructorArgs, info.Value, info.GasLimit, cancellationToken);
            ...
            //code-step: 1.5 - Extract deployment transaction hash record from result
            var deploy = new Dictionary<string, object> { { "transactionHash", r.TransactionHash } };
            ...
            outputData["operation"] = "deploy";

            //code-step: 1.7 - Convert deployment record to standard items array
            outputData.TryGetValue(ExecutionConstants.OutputFieldNameConstants.CONST_items, out var existingItemsValue);
            outputData[ExecutionConstants.OutputFieldNameConstants.CONST_items] = ApplyOutputItemsMerge(existingItemsValue, WrapJsonIntoItems(deploy, nodeExecutionContext));

            //code-step: 1.8 - Write output (handles TargetDataPath writes + items downstream)
            return await WriteOutputData(ExecutionConstants.OutputPorts.Success, outputData, deploy, nodeExecutionContext, cancellationToken);
        }
        catch (OperationCanceledException) { throw; }
        catch (Exception ex)
        {
            //code-step: 1.9 - Catch exceptions and return error with context
            return resultManager.SetResultAsError(ExecutionConstants.OutputPorts.Error, $"contract/deploy failed: {ex.Message}", this);
        }
    }
}
