...
{
    private async Task<NodeExecutionResult> _Ethereum_Contract_Write_Async(
        NodeExecutionContext nodeExecutionContext,
        CancellationToken cancellationToken = default)
    {
        //code-step: 1.1 - Validate settings exist and cast to ContractWriteInfo
        if (mySettings?.ActiveInfo is not ContractWriteInfo info)
        ...
        try
        {
            //code-step: 1.3 - Resolve signing key and call Ethereum contract service to send the transaction
            var (privateKey, credError) = await _ResolveSigningKeyAsync(cancellationToken);
            if (credError is not null)
                return credError;

            var r = await _contractService.WriteAsync(info.Network, privateKey!, info.ContractAddress!, info.Abi!, info.FunctionName!, info.FunctionArgs, info.Value, info.GasLimit, cancellationToken);
            ...
            //code-step: 1.5 - Extract transaction hash record from result
            var write = new Dictionary<string, object> { { "transactionHash", r.TransactionHash } };
            ...
            outputData["operation"] = "write";

            //code-step: 1.7 - Convert transaction hash record to standard items array
            outputData.TryGetValue(ExecutionConstants.OutputFieldNameConstants.CONST_items, out var existingItemsValue);
            outputData[ExecutionConstants.OutputFieldNameConstants.CONST_items] = ApplyOutputItemsMerge(existingItemsValue, WrapJsonIntoItems(write, nodeExecutionContext));

            //code-step: 1.8 - Write output (handles TargetDataPath writes + items downstream)
            return await WriteOutputData(ExecutionConstants.OutputPorts.Success, outputData, write, nodeExecutionContext, cancellationToken);
        }
        catch (OperationCanceledException) { throw; }
        catch (Exception ex)
        {
            //code-step: 1.9 - Catch exceptions and return error with context
            return resultManager.SetResultAsError(ExecutionConstants.OutputPorts.Error, $"contract/write failed for {info.ContractAddress}.{info.FunctionName}: {ex.Message}", this);
        }
    }
}
