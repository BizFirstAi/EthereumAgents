...
{
    private async Task<NodeExecutionResult> _Ethereum_Contract_Read_Async(
        NodeExecutionContext nodeExecutionContext,
        CancellationToken cancellationToken = default)
    {
        //code-step: 1.1 - Validate settings exist and cast to ContractReadInfo
        if (mySettings?.ActiveInfo is not ContractReadInfo info)
        ...
        try
        {
            //code-step: 1.3 - Call Ethereum contract service to read the function
            var r = await _contractService.ReadAsync(info.Network, info.ContractAddress!, info.Abi!, info.FunctionName!, info.FunctionArgs, info.Block, cancellationToken);
            ...
            //code-step: 1.5 - Extract decoded read record from result
            var read = new Dictionary<string, object>
            {
                { "result", r.Result ?? string.Empty },
                { "decodedResult", r.DecodedResult },
            };
            ...
            outputData["operation"] = "read";

            //code-step: 1.7 - Convert read record to standard items array
            outputData.TryGetValue(ExecutionConstants.OutputFieldNameConstants.CONST_items, out var existingItemsValue);
            outputData[ExecutionConstants.OutputFieldNameConstants.CONST_items] = ApplyOutputItemsMerge(existingItemsValue, WrapJsonIntoItems(read, nodeExecutionContext));

            //code-step: 1.8 - Write output (handles TargetDataPath writes + items downstream)
            return await WriteOutputData(ExecutionConstants.OutputPorts.Success, outputData, read, nodeExecutionContext, cancellationToken);
        }
        catch (OperationCanceledException) { throw; }
        catch (Exception ex)
        {
            //code-step: 1.9 - Catch exceptions and return error with context
            return resultManager.SetResultAsError(ExecutionConstants.OutputPorts.Error, $"contract/read failed for {info.ContractAddress}.{info.FunctionName}: {ex.Message}", this);
        }
    }
}
