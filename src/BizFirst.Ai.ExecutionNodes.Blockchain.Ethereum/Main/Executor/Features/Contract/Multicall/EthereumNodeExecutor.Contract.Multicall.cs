...
{
    private async Task<NodeExecutionResult> _Ethereum_Contract_Multicall_Async(
        NodeExecutionContext nodeExecutionContext,
        CancellationToken cancellationToken = default)
    {
        //code-step: 1.1 - Validate settings exist and cast to ContractMulticallInfo
        if (mySettings?.ActiveInfo is not ContractMulticallInfo info)
        ...
        try
        {
            //code-step: 1.3 - Call Ethereum contract service to batch-run the read calls
            var r = await _contractService.MulticallAsync(info.Network, info.Calls!, cancellationToken);
            ...
            //code-step: 1.5 - Extract per-call results list from result
            ...
            outputData["operation"] = "multicall";
            outputData["count"] = results.Count;

            //code-step: 1.7 - Convert per-call results to standard items array
            outputData.TryGetValue(ExecutionConstants.OutputFieldNameConstants.CONST_items, out var existingItemsValue);
            outputData[ExecutionConstants.OutputFieldNameConstants.CONST_items] = ApplyOutputItemsMerge(existingItemsValue, WrapJsonIntoItems(results, nodeExecutionContext));

            //code-step: 1.8 - Write output (handles TargetDataPath writes + items downstream)
            return await WriteOutputData(ExecutionConstants.OutputPorts.Success, outputData, results, nodeExecutionContext, cancellationToken);
        }
        catch (OperationCanceledException) { throw; }
        catch (Exception ex)
        {
            //code-step: 1.9 - Catch exceptions and return error with context
            return resultManager.SetResultAsError(ExecutionConstants.OutputPorts.Error, $"contract/multicall failed: {ex.Message}", this);
        }
    }
}
