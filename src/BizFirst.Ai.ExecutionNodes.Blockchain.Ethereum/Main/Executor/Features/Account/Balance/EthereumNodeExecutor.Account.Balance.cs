...
{
    private async Task<NodeExecutionResult> _Ethereum_Account_Balance_Async(
        NodeExecutionContext nodeExecutionContext,
        CancellationToken cancellationToken = default)
    {
        //code-step: 1.1 - Validate settings exist and cast to AccountBalanceInfo
        if (mySettings?.ActiveInfo is not AccountBalanceInfo info)
        ...
        try
        {
            //code-step: 1.3 - Call Ethereum account service to fetch balance
            var r = await _accountService.GetBalanceAsync(info.Network, info.Address!, info.Format, info.Block, cancellationToken);
        ...
        catch (Exception ex)
        {
            ...
            return resultManager.SetResultAsError(ExecutionConstants.OutputPorts.Error, $"account/balance failed for {info.Address}: {ex.Message}", this);
        }
    }
}
