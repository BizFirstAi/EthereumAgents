...
{
    private async Task<NodeExecutionResult> _Ethereum_Account_Nonce_Async(
        NodeExecutionContext nodeExecutionContext,
        CancellationToken cancellationToken = default)
    {
        //code-step: 1.1 - Validate settings exist and cast to AccountNonceInfo
        if (mySettings?.ActiveInfo is not AccountNonceInfo info)
        ...
        try
        {
            //code-step: 1.3 - Call Ethereum account service to fetch nonce
            var r = await _accountService.GetNonceAsync(info.Network, info.Address!, info.Block, cancellationToken);
            ...
            //code-step: 1.5 - Extract nonce record from result
            var nonce = new Dictionary<string, object> { { "nonce", r.Nonce } };
            ...
            outputData["operation"] = "nonce";

            //code-step: 1.7 - Convert nonce record to standard items array
            outputData.TryGetValue(ExecutionConstants.OutputFieldNameConstants.CONST_items, out var existingItemsValue);
            outputData[ExecutionConstants.OutputFieldNameConstants.CONST_items] = ApplyOutputItemsMerge(existingItemsValue, WrapJsonIntoItems(nonce, nodeExecutionContext));

            //code-step: 1.8 - Write output (handles TargetDataPath writes + items downstream)
            return await WriteOutputData(ExecutionConstants.OutputPorts.Success, outputData, nonce, nodeExecutionContext, cancellationToken);
        }
        catch (OperationCanceledException) { throw; }
        catch (Exception ex)
        {
            //code-step: 1.9 - Catch exceptions and return error with context
            return resultManager.SetResultAsError(ExecutionConstants.OutputPorts.Error, $"account/nonce failed for {info.Address}: {ex.Message}", this);
        }
    }
}
