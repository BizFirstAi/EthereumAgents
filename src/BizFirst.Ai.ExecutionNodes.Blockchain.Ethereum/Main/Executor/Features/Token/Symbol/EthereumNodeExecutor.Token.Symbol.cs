...
{
    private async Task<NodeExecutionResult> _Ethereum_Token_Symbol_Async(
        NodeExecutionContext nodeExecutionContext,
        CancellationToken cancellationToken = default)
    {
        //code-step: 1.1 - Validate settings exist and cast to TokenSymbolInfo
        if (mySettings?.ActiveInfo is not TokenSymbolInfo info)
        ...
        try
        {
            //code-step: 1.3 - Call Ethereum token service to fetch token symbol
            var r = await _tokenService.GetSymbolAsync(info.Network, info.TokenAddress!, cancellationToken);
            ...
            //code-step: 1.5 - Extract token symbol record from result
            var symbol = new Dictionary<string, object> { { "symbol", r.Symbol } };
            ...
            outputData["operation"] = "symbol";

            //code-step: 1.7 - Convert token symbol record to standard items array
            outputData.TryGetValue(ExecutionConstants.OutputFieldNameConstants.CONST_items, out var existingItemsValue);
            outputData[ExecutionConstants.OutputFieldNameConstants.CONST_items] = ApplyOutputItemsMerge(existingItemsValue, WrapJsonIntoItems(symbol, nodeExecutionContext));

            //code-step: 1.8 - Write output (handles TargetDataPath writes + items downstream)
            return await WriteOutputData(ExecutionConstants.OutputPorts.Success, outputData, symbol, nodeExecutionContext, cancellationToken);
        }
        catch (OperationCanceledException) { throw; }
        catch (Exception ex)
        {
            //code-step: 1.9 - Catch exceptions and return error with context
            return resultManager.SetResultAsError(ExecutionConstants.OutputPorts.Error, $"token/symbol failed for {info.TokenAddress}: {ex.Message}", this);
        }
    }
}
