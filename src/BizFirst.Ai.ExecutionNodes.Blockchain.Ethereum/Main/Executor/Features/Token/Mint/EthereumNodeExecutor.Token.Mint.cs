...
{
    private async Task<NodeExecutionResult> _Ethereum_Token_Mint_Async(
        NodeExecutionContext nodeExecutionContext,
        CancellationToken cancellationToken = default)
    {
        //code-step: 1.1 - Validate settings exist and cast to TokenMintInfo
        if (mySettings?.ActiveInfo is not TokenMintInfo info)
        ...
        try
        {
            //code-step: 1.3 - Resolve signing credential and call Ethereum token service to mint (no standard signature — see addendum v3)
            ...
            var r = await _tokenService.MintAsync(info.Network, privateKey!, info.TokenAddress!, info.To!, amount, info.FunctionName, cancellationToken);
        ...
        catch (Exception ex)
        {
            ...
            return resultManager.SetResultAsError(ExecutionConstants.OutputPorts.Error, $"token/mint failed for {info.TokenAddress} to {info.To}: {ex.Message}", this);
        }
    }
}
