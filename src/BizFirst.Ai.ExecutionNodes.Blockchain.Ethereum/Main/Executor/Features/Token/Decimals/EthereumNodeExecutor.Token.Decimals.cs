...
{
    private async Task<NodeExecutionResult> _Ethereum_Token_Decimals_Async(
        NodeExecutionContext nodeExecutionContext,
        CancellationToken cancellationToken = default)
    {
        //code-step: 1.1 - Validate settings exist and cast to TokenDecimalsInfo
        if (mySettings?.ActiveInfo is not TokenDecimalsInfo info)
        ...
        try
        {
            //code-step: 1.3 - Call Ethereum token service to fetch decimals
            var r = await _tokenService.GetDecimalsAsync(info.Network, info.TokenAddress!, cancellationToken);
            ...
            //code-step: 1.5 - Extract decimals record from result
            var decimals = new Dictionary<string, object> { { "decimals", r.Decimals } };
            ...
            outputData["operation"] = "decimals";

            //code-step: 1.7 - Convert decimals record to standard items array
            outputData.TryGetValue(ExecutionConstants.OutputFieldNameConstants.CONST_items, out var existingItemsValue);
            outputData[ExecutionConstants.OutputFieldNameConstants.CONST_items] = ApplyOutputItemsMerge(existingItemsValue, WrapJsonIntoItems(decimals, nodeExecutionContext));

            //code-step: 1.8 - Write output (handles TargetDataPath writes + items downstream)
            return await WriteOutputData(ExecutionConstants.OutputPorts.Success, outputData, decimals, nodeExecutionContext, cancellationToken);
        }
        catch (OperationCanceledException) { throw; }
        catch (Exception ex)
        {
            //code-step: 1.9 - Catch exceptions and return error with context
            return resultManager.SetResultAsError(ExecutionConstants.OutputPorts.Error, $"token/decimals failed for {info.TokenAddress}: {ex.Message}", this);
        }
    }
}
