...
{
    private async Task<NodeExecutionResult> _Ethereum_Token_Burn_Async(
        NodeExecutionContext nodeExecutionContext,
        CancellationToken cancellationToken = default)
    {
        //code-step: 1.1 - Validate settings exist and cast to TokenBurnInfo
        if (mySettings?.ActiveInfo is not TokenBurnInfo info)
        ...
        try
        {
            //code-step: 1.3 - Resolve signing credential and call Ethereum token service to burn (ERC20Burnable convention)
            var (privateKey, credError) = await _ResolveSigningKeyAsync(cancellationToken);
            if (credError is not null)
                return credError;

            var amount = BigInteger.Parse(info.Amount);
            var r = await _tokenService.BurnAsync(info.Network, privateKey!, info.TokenAddress!, info.From, amount, cancellationToken);
            ...
            var burned = new Dictionary<string, object> { { "txHash", r.TxHash } };
            ...
            outputData[ExecutionConstants.OutputFieldNameConstants.CONST_items] = ApplyOutputItemsMerge(existingItemsValue, WrapJsonIntoItems(burned, nodeExecutionContext));

            //code-step: 1.8 - Write output (handles TargetDataPath writes + items downstream)
            return await WriteOutputData(ExecutionConstants.OutputPorts.Success, outputData, burned, nodeExecutionContext, cancellationToken);
        }
        catch (OperationCanceledException) { throw; }
        catch (Exception ex)
        {
            //code-step: 1.9 - Catch exceptions and return error with context
            return resultManager.SetResultAsError(ExecutionConstants.OutputPorts.Error, $"token/burn failed for {info.TokenAddress}: {ex.Message}", this);
        }
    }
}
