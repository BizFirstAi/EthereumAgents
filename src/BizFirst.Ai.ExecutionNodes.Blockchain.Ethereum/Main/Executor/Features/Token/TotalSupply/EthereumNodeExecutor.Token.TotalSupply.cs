...
{
    private async Task<NodeExecutionResult> _Ethereum_Token_TotalSupply_Async(
        NodeExecutionContext nodeExecutionContext,
        CancellationToken cancellationToken = default)
    {
        //code-step: 1.1 - Validate settings exist and cast to TokenTotalSupplyInfo
        if (mySettings?.ActiveInfo is not TokenTotalSupplyInfo info)
        ...
        try
        {
            //code-step: 1.3 - Call Ethereum token service to fetch total supply
            var r = await _tokenService.GetTotalSupplyAsync(info.Network, info.TokenAddress!, info.FormatDecimals, cancellationToken);
            ...
            //code-step: 1.5 - Extract total supply record from result
            var totalSupply = new Dictionary<string, object>
            {
                { "rawTotalSupply", r.RawTotalSupply },
                { "formattedTotalSupply", r.FormattedTotalSupply! },
            };
            ...
            outputData["operation"] = "totalSupply";

            //code-step: 1.7 - Convert total supply record to standard items array
            outputData.TryGetValue(ExecutionConstants.OutputFieldNameConstants.CONST_items, out var existingItemsValue);
            outputData[ExecutionConstants.OutputFieldNameConstants.CONST_items] = ApplyOutputItemsMerge(existingItemsValue, WrapJsonIntoItems(totalSupply, nodeExecutionContext));

            //code-step: 1.8 - Write output (handles TargetDataPath writes + items downstream)
            return await WriteOutputData(ExecutionConstants.OutputPorts.Success, outputData, totalSupply, nodeExecutionContext, cancellationToken);
        }
        catch (OperationCanceledException) { throw; }
        catch (Exception ex)
        {
            //code-step: 1.9 - Catch exceptions and return error with context
            return resultManager.SetResultAsError(ExecutionConstants.OutputPorts.Error, $"token/totalSupply failed for {info.TokenAddress}: {ex.Message}", this);
        }
    }
}
