...
{
    private async Task<NodeExecutionResult> _Ethereum_Token_Allowance_Async(
        NodeExecutionContext nodeExecutionContext,
        CancellationToken cancellationToken = default)
    {
        //code-step: 1.1 - Validate settings exist and cast to TokenAllowanceInfo
        if (mySettings?.ActiveInfo is not TokenAllowanceInfo info)
        ...
        try
        {
            //code-step: 1.3 - Call Ethereum token service to fetch spender allowance
            var r = await _tokenService.GetAllowanceAsync(info.Network, info.TokenAddress!, info.Owner!, info.Spender!, info.FormatDecimals, cancellationToken);
            ...
            //code-step: 1.5 - Extract allowance record from result
            var allowance = new Dictionary<string, object>
            {
                { "rawAllowance", r.RawAllowance },
                { "formattedAllowance", r.FormattedAllowance! },
            };
            ...
            outputData["operation"] = "allowance";

            //code-step: 1.7 - Convert allowance record to standard items array
            outputData.TryGetValue(ExecutionConstants.OutputFieldNameConstants.CONST_items, out var existingItemsValue);
            outputData[ExecutionConstants.OutputFieldNameConstants.CONST_items] = ApplyOutputItemsMerge(existingItemsValue, WrapJsonIntoItems(allowance, nodeExecutionContext));

            //code-step: 1.8 - Write output (handles TargetDataPath writes + items downstream)
            return await WriteOutputData(ExecutionConstants.OutputPorts.Success, outputData, allowance, nodeExecutionContext, cancellationToken);
        }
        catch (OperationCanceledException) { throw; }
        catch (Exception ex)
        {
            //code-step: 1.9 - Catch exceptions and return error with context
            return resultManager.SetResultAsError(ExecutionConstants.OutputPorts.Error, $"token/allowance failed for {info.TokenAddress} owner {info.Owner} spender {info.Spender}: {ex.Message}", this);
        }
    }
}
