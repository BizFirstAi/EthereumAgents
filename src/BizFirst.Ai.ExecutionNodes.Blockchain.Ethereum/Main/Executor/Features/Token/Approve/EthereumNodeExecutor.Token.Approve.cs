...
{
    private async Task<NodeExecutionResult> _Ethereum_Token_Approve_Async(
        NodeExecutionContext nodeExecutionContext,
        CancellationToken cancellationToken = default)
    {
        //code-step: 1.1 - Validate settings exist and cast to TokenApproveInfo
        if (mySettings?.ActiveInfo is not TokenApproveInfo info)
        ...
        // Resolve the wallet signing key from the vault before attempting the approval.
        ...
        try
        {
            //code-step: 1.3 - Call Ethereum token service to approve spender allowance
            var r = await _tokenService.ApproveAsync(
                info.Network, privateKey!, info.TokenAddress!, info.Spender!, System.Numerics.BigInteger.Parse(info.Amount), cancellationToken);
            ...
            //code-step: 1.5 - Extract token approval record from result
            var approval = new Dictionary<string, object> { { "txHash", r.TxHash } };
            ...
            outputData["operation"] = "approve";

            //code-step: 1.7 - Convert token approval record to standard items array
            outputData.TryGetValue(ExecutionConstants.OutputFieldNameConstants.CONST_items, out var existingItemsValue);
            outputData[ExecutionConstants.OutputFieldNameConstants.CONST_items] = ApplyOutputItemsMerge(existingItemsValue, WrapJsonIntoItems(approval, nodeExecutionContext));

            //code-step: 1.8 - Write output (handles TargetDataPath writes + items downstream)
            return await WriteOutputData(ExecutionConstants.OutputPorts.Success, outputData, approval, nodeExecutionContext, cancellationToken);
        }
        catch (OperationCanceledException) { throw; }
        catch (Exception ex)
        {
            //code-step: 1.9 - Catch exceptions and return error with context
            return resultManager.SetResultAsError(ExecutionConstants.OutputPorts.Error, $"token/approve failed for {info.TokenAddress} spender {info.Spender}: {ex.Message}", this);
        }
    }
}
