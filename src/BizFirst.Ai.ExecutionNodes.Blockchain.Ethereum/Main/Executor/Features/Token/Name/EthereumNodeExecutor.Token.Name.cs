...
{
    private async Task<NodeExecutionResult> _Ethereum_Token_Name_Async(
        NodeExecutionContext nodeExecutionContext,
        CancellationToken cancellationToken = default)
    {
        //code-step: 1.1 - Validate settings exist and cast to TokenNameInfo
        if (mySettings?.ActiveInfo is not TokenNameInfo info)
        ...
        try
        {
            //code-step: 1.3 - Call Ethereum token service to fetch token name
            var r = await _tokenService.GetNameAsync(info.Network, info.TokenAddress!, cancellationToken);
            ...
            //code-step: 1.5 - Extract token name record from result
            var name = new Dictionary<string, object> { { "name", r.Name } };
            ...
            outputData["operation"] = "name";

            //code-step: 1.7 - Convert token name record to standard items array
            outputData.TryGetValue(ExecutionConstants.OutputFieldNameConstants.CONST_items, out var existingItemsValue);
            outputData[ExecutionConstants.OutputFieldNameConstants.CONST_items] = ApplyOutputItemsMerge(existingItemsValue, WrapJsonIntoItems(name, nodeExecutionContext));

            //code-step: 1.8 - Write output (handles TargetDataPath writes + items downstream)
            return await WriteOutputData(ExecutionConstants.OutputPorts.Success, outputData, name, nodeExecutionContext, cancellationToken);
        }
        catch (OperationCanceledException) { throw; }
        catch (Exception ex)
        {
            //code-step: 1.9 - Catch exceptions and return error with context
            return resultManager.SetResultAsError(ExecutionConstants.OutputPorts.Error, $"token/name failed for {info.TokenAddress}: {ex.Message}", this);
        }
    }
}
