...
{
    private async Task<NodeExecutionResult> _Ethereum_Token_Balance_Async(
        NodeExecutionContext nodeExecutionContext,
        CancellationToken cancellationToken = default)
    {
        //code-step: 1.1 - Validate settings exist and cast to TokenBalanceInfo
        if (mySettings?.ActiveInfo is not TokenBalanceInfo info)
        ...
        try
        {
            //code-step: 1.3 - Call Ethereum token service to fetch token balance
            var r = await _tokenService.GetBalanceAsync(info.Network, info.TokenAddress!, info.OwnerAddress!, info.FormatDecimals, cancellationToken);
            ...
            //code-step: 1.5 - Extract token balance record from result
            ...
            outputData["operation"] = "balance";

            //code-step: 1.7 - Convert token balance record to standard items array
            outputData.TryGetValue(ExecutionConstants.OutputFieldNameConstants.CONST_items, out var existingItemsValue);
            outputData[ExecutionConstants.OutputFieldNameConstants.CONST_items] = ApplyOutputItemsMerge(existingItemsValue, WrapJsonIntoItems(balance, nodeExecutionContext));

            //code-step: 1.8 - Write output (handles TargetDataPath writes + items downstream)
            return await WriteOutputData(ExecutionConstants.OutputPorts.Success, outputData, balance, nodeExecutionContext, cancellationToken);
        }
        catch (OperationCanceledException) { throw; }
        catch (Exception ex)
        {
            //code-step: 1.9 - Catch exceptions and return error with context
            return resultManager.SetResultAsError(ExecutionConstants.OutputPorts.Error, $"token/balance failed for {info.TokenAddress}/{info.OwnerAddress}: {ex.Message}", this);
        }
    }
}
