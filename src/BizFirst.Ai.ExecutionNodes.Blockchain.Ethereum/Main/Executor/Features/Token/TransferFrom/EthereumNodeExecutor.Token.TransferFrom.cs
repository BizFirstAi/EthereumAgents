...
{
    private async Task<NodeExecutionResult> _Ethereum_Token_TransferFrom_Async(
        NodeExecutionContext nodeExecutionContext,
        CancellationToken cancellationToken = default)
    {
        //code-step: 1.1 - Validate settings exist and cast to TokenTransferFromInfo
        if (mySettings?.ActiveInfo is not TokenTransferFromInfo info)
        ...
        try
        {
            //code-step: 1.3 - Call Ethereum token service to transfer tokens via allowance
            var r = await _tokenService.TransferFromAsync(
                info.Network, privateKey!, info.TokenAddress!, info.From!, info.To!, System.Numerics.BigInteger.Parse(info.Amount), cancellationToken);
            ...
            //code-step: 1.5 - Extract token transferFrom record from result
            ...
            //code-step: 1.7 - Convert token transferFrom record to standard items array
        ...
        catch (Exception ex)
        {
            ...
            return resultManager.SetResultAsError(ExecutionConstants.OutputPorts.Error, $"token/transferFrom failed for {info.TokenAddress} from {info.From} to {info.To}: {ex.Message}", this);
        }
    }
}
