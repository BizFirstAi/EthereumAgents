...
{
    private async Task<NodeExecutionResult> _Ethereum_Transaction_Pending_Async(
        NodeExecutionContext nodeExecutionContext,
        CancellationToken cancellationToken = default)
    {
        //code-step: 1.1 - Validate settings exist and cast to TransactionPendingInfo
        if (mySettings?.ActiveInfo is not TransactionPendingInfo info)
        ...
        try
        {
            //code-step: 1.3 - Call Ethereum transaction service to fetch pending transactions
            var r = await _transactionService.GetPendingTransactionsAsync(info.Network, info.AddressFilter, cancellationToken);
            ...
            //code-step: 1.5 - Extract pending transaction list from result
            ...
            outputData["operation"] = "pending";
            outputData["count"] = transactions.Count;

            //code-step: 1.7 - Convert pending transaction list to standard items array
            outputData.TryGetValue(ExecutionConstants.OutputFieldNameConstants.CONST_items, out var existingItemsValue);
            outputData[ExecutionConstants.OutputFieldNameConstants.CONST_items] = ApplyOutputItemsMerge(existingItemsValue, WrapJsonIntoItems(transactions, nodeExecutionContext));

            //code-step: 1.8 - Write output (handles TargetDataPath writes + items downstream)
            return await WriteOutputData(ExecutionConstants.OutputPorts.Success, outputData, transactions, nodeExecutionContext, cancellationToken);
        }
        catch (OperationCanceledException) { throw; }
        catch (Exception ex)
        {
            //code-step: 1.9 - Catch exceptions and return error with context
            return resultManager.SetResultAsError(ExecutionConstants.OutputPorts.Error, $"transaction/pending failed on {info.Network ?? "default"}: {ex.Message}", this);
        }
    }
}
