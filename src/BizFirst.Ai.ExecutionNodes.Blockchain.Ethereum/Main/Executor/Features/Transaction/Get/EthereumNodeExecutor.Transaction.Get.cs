...
{
    private async Task<NodeExecutionResult> _Ethereum_Transaction_Get_Async(
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            //code-step: 1.3 - Call Ethereum transaction service to fetch transaction
            var r = await _transactionService.GetTransactionAsync(info.Network, info.Hash!, cancellationToken);
        ...
        catch (Exception ex)
        {
            ...
            return resultManager.SetResultAsError(ExecutionConstants.OutputPorts.Error, $"transaction/get failed for {info.Hash}: {ex.Message}", this);
        }
    }
}
