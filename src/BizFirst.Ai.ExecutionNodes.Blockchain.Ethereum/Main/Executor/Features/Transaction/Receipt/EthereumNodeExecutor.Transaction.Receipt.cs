...
{
    private async Task<NodeExecutionResult> _Ethereum_Transaction_Receipt_Async(
        NodeExecutionContext nodeExecutionContext,
        CancellationToken cancellationToken = default)
    {
        //code-step: 1.1 - Validate settings exist and cast to TransactionReceiptInfo
        if (mySettings?.ActiveInfo is not TransactionReceiptInfo info)
        ...
        try
        {
            //code-step: 1.3 - Call Ethereum transaction service to fetch receipt
            var r = await _transactionService.GetReceiptAsync(info.Network, info.Hash!, cancellationToken);
            ...
            //code-step: 1.5 - Extract receipt record from result
            ...
            outputData["operation"] = "receipt";
            outputData["logs"] = r.Logs;

            //code-step: 1.7 - Convert receipt record to standard items array
            outputData.TryGetValue(ExecutionConstants.OutputFieldNameConstants.CONST_items, out var existingItemsValue);
            outputData[ExecutionConstants.OutputFieldNameConstants.CONST_items] = ApplyOutputItemsMerge(existingItemsValue, WrapJsonIntoItems(receipt, nodeExecutionContext));

            //code-step: 1.8 - Write output (handles TargetDataPath writes + items downstream)
            return await WriteOutputData(ExecutionConstants.OutputPorts.Success, outputData, receipt, nodeExecutionContext, cancellationToken);
        }
        catch (OperationCanceledException) { throw; }
        catch (Exception ex)
        {
            //code-step: 1.9 - Catch exceptions and return error with context
            return resultManager.SetResultAsError(ExecutionConstants.OutputPorts.Error, $"transaction/receipt failed for {info.Hash}: {ex.Message}", this);
        }
    }
}
