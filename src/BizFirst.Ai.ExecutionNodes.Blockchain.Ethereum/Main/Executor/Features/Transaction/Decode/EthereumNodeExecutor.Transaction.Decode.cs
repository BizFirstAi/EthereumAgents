...
{
    private async Task<NodeExecutionResult> _Ethereum_Transaction_Decode_Async(
        NodeExecutionContext nodeExecutionContext,
        CancellationToken cancellationToken = default)
    {
        //code-step: 1.1 - Validate settings exist and cast to TransactionDecodeInfo
        if (mySettings?.ActiveInfo is not TransactionDecodeInfo info)
        ...
        try
        {
            //code-step: 1.3 - Call Ethereum transaction service to decode transaction data
            var r = await _transactionService.DecodeTransactionDataAsync(info.Data!, info.Abi, cancellationToken);
            ...
            //code-step: 1.5 - Extract decoded record from result
            ...
            outputData["operation"] = "decode";

            //code-step: 1.7 - Convert decoded record to standard items array
        ...
        catch (Exception ex)
        {
            ...
            return resultManager.SetResultAsError(ExecutionConstants.OutputPorts.Error, $"transaction/decode failed for data {info.Data}: {ex.Message}", this);
        }
    }
}
